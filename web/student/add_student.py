...
class Add_Student(Resource):
    def __init__(self, client, db, bde_collection, manager_collection, mentor_collection,collection):
        super().__init__()
        self.client = client
        self.db_name = db
    ...
    '''def post(self):
        ...
        std_BatchNo = request.json.get('batchNo')
        ...
        passwod=''
        for _ in range(2):
            passwod+=random.choice(u_c)
            passwod+=str(random.randint(0,9))
            passwod+=random.choice(l_c)
        ...
        print('add-student:-----',student_data)
        result = self.collection.insert_one(student_data)
        student_data['_id'] = str(result.inserted_id)

        self.send_email(email,studentId,passwod)

        return {"message": "student added successful"}, 200


#multiple data 
    def post(self):
        excel_data = request.json.get('excelData')
        ...
        for student in excel_data:
            ...
            if not all([studentId, batchNo, email, parentNo]):
            ...
            self.send_email(email,studentId,password)
